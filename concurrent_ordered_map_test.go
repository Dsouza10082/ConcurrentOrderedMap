...
func TestUpdateOperations(t *testing.T) {
	...
	// Test SetOrUpdate
	err = m.SetOrUpdate("new", TestStruct{ID: 2, Name: "New", Score: 5.0}, func(v *TestStruct) error {
		v.Score = 15.0
		return nil
	})
...
func TestGetOrDefault(t *testing.T) {
	...
	val := m.GetOrDefault("exists", 0)
	if val != 42 {
		t.Errorf("Expected 42, got %d", val)
	}
	
	val = m.GetOrDefault("notexists", 100)
	if val != 100 {
		t.Errorf("Expected default value 100, got %d", val)
	}
}
